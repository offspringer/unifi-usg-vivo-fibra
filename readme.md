...
1. Rotas estáticas: deve-se verificar se algumas rotas estáticas fornecidades pela Vivo Fibra foram alteradas. Infelizmente, não há muito o que fazer nesta parte, a não ser comparar com as que o roteador original recebe através do protocolo de atualização TR-069. Vale lembrar que algumas rotas estáticas são configuradas utilizando como `next-hop` os gateways da Vivo. Em 2026 estes gateways mudaram e, portanto, foi necessário obter os IPs dos novos gateways de IPTV e VoIP através do comando `show dhcp client leases` dentro do próprio USG (o valor que estiver em `router` é o gateway que estamos procurando);
